...
export default class TodoPlugin extends Plugin {
	...
	private onFileModified = debounce(
		(file: TFile) => { void this.scanFileForTodos(file); },
		1000,
		true
	);
	...
	async onload() {
		...
		this.addRibbonIcon('check-square', 'Cairn', () => { void this.openTodoPanel(); });
		this.addSettingTab(new TodoSettingTab(this.app, this));

		this.app.workspace.onLayoutReady(() => {
			void this.loadAllTodos().then(() => this.refreshTodoView());
		});
	...
	onunload() { /* intentionally empty — Obsidian handles leaf cleanup */ }
